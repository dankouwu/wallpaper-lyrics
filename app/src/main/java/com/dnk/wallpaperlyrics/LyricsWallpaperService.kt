...
class LyricsWallpaperService : WallpaperService() {
    ...
    companion object {
        ...
        internal const val AURORA_SHADER = """
            ...
            float3 rgbToOklab(float3 srgb) {
                float3 c = srgbToLinear(srgb);
            ...
            half4 main(float2 fragCoord) {
                ...
                // A muted cover reads flat where a vivid one already carries itself, so the
                // lift is gated to a chroma band: greys and near blacks keep their contrast
                // against the lyrics, and a cover that is already saturated is left alone.
                float3 lab = rgbToOklab(float3(color.rgb));
                float chroma = length(lab.yz);
                float gate = smoothstep(0.02, 0.06, chroma) * (1.0 - smoothstep(0.155, 0.20, chroma));
                if (gate > 0.0) {
                    lab.x = min(lab.x * (1.0 + 0.24 * gate), 1.0);
                    color.rgb = half3(oklabToRgb(lab));
                }
